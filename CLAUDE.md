...
任务布置在 docs/tasks.md，只能由我手动修改;
docs/plan.md里面写阶段任务安排、目前任务安排、遗留问题（尽可能提供解决方案，或者需要询问）和难点；可以随意修改
arch.md里面开头整体架构和运行流程，按照阶段划分，每个阶段有有内部架构，维护的数据结构，运行机制（辅以例子），以及所有架构选择附上选择理由；可以随意修改；另外arch内容只作为参考作用，不是一定要按照，当我的实现和arch不一致时，应该比较优劣而不是盲目跟随arch
